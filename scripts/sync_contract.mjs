...
export const syncContract = async () => {
  if (process.env.HOGMP_CONTRACT_ROOT) return path.resolve(process.env.HOGMP_CONTRACT_ROOT);
  const apiRoot = (process.env.HOGMP_API_URL || "https://api.mafiahub.dev").replace(/\/graphql\/?$/, "").replace(/\/$/, "");
  const channel = process.env.HOGMP_CONTRACT_CHANNEL ?? "testing";
  const requestedRevision = process.env.HOGMP_CONTRACT_REVISION;
  const routeRoot = `${apiRoot}/documentation-contracts/hogwarts-mp`;
  const manifestUrl = requestedRevision ? `${routeRoot}/releases/${requestedRevision}/manifest.json` : `${routeRoot}/${channel}/manifest.json`;
  ...
  if (manifest.schemaVersion !== 1 || manifest.slug !== "hogwarts-mp" || manifest.channel !== channel || !/^[a-f0-9]{64}$/.test(manifest.revision ?? "") || !/^[A-Za-z0-9][A-Za-z0-9._+-]{0,63}$/.test(manifest.version ?? "") || !Array.isArray(manifest.files)) throw new Error("Services returned an invalid scripting contract manifest");
  ...
  try {
    for (const file of manifest.files) {
      const fileUrl = `${routeRoot}/releases/${manifest.revision}/files/${file.path.split("/").map(encodeURIComponent).join("/")}`;
