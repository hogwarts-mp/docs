...
const generate = async (output) => {
  ...
  const args = ["docs", "generate", "--config", configPath, "--out", output, "--content-root", root, "--content-revision", contentRevision, "--site-url", process.env.HOGMP_DOCS_SITE_URL ?? "http://127.0.0.1:4321", "--base-path", process.env.HOGMP_DOCS_BASE_PATH ?? "/"];
  ...
  siteManifest.sources.reference.channel = process.env.HOGMP_CONTRACT_CHANNEL ?? "testing";
